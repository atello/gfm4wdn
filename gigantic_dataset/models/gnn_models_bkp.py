...
class StructuralEncoder(Module):
    def __init__(self, in_dim, out_dim, hidden_channels):
        ...
        self.encoder = GATResMeanConv(in_dim=hidden_channels*2, out_dim=out_dim, nc=hidden_channels, add_self_loops=True)
...
class DualEncoderGFM(Module):
    def __init__(self, out_dim: int, hidden_channels: int, num_blocks: int, **kwargs):
        ...
        self.pe = PositionalEncoding(out_channels=hidden_channels)
        self.stem_structure = Linear(1, hidden_channels)

        self.structure_encoder = GATResMeanConv(in_dim=hidden_channels*2, out_dim=2, nc=hidden_channels, num_blocks=num_blocks, edge_dim=None, add_self_loops=False)
    ...
    def forward(self, x_struct: Tensor, x_func: Tensor,
        ...
        x_pe = self.pe(x_struct)
        x_struct = self.stem_structure(x_struct)
        x_struct = torch.concat([x_pe, x_struct], dim=1)
        ...
        # struct_bias = self.struct_bias(h_degree)
...
class LoadDualEncoderGFMFineTune(LoadModelProto):
    def __call__(
        ...
        setattr(model, "name", model_configs[0].name)
        if model_configs[0].weight_path != "" and os.path.exists(
                model_configs[0].weight_path):
            models, _ = load_weights(path=model_configs[0].weight_path, models=[model],
                                     load_keys=[model_configs[0].name])
            model = models[0]
        modules.append(model)
