...
from gigantic_dataset.core.train import train, eval, TrainOneEpochDegreeEncoder, TestOneEpochDegreeEncoder, \
    WandbStartProfiler, SemiSingleNNDegForward, TrainOneEpoch, TestOneEpoch, ForwardNNDegreeWOfeat

from gigantic_dataset.core.run import extract_dataset_name
from gigantic_dataset.utils.pretrain_utils import LambdaScheduler, DegreeNormalizer
...
from gigantic_dataset.models.gnn_models import LoadGATRes, LoadStructuralEncoder, LoadGraphWaterSE
...
def pretraining_ndegree(
    ...
    func_ref = FuncRef(
        ...
        # forward_fn=SemiSingleNNDegForward(),
        forward_fn=ForwardNNDegreeWOfeat(),
        train_one_epoch_fn=TrainOneEpochDegreeEncoder(),
        test_one_epoch_fn=TestOneEpochDegreeEncoder(),
        train_fn=train,
        eval_fn=eval,
        ...
        load_models=LoadStructuralEncoder(),  # <------------------------------------ CHECK
        # load_models=LoadGATRes(),  # <------------------------------------ CHECK
    ...
    # start model
    models = func_ref.load_models(in_dims=[4], out_dims=[out_node_dim], edge_dim=3)

    lambda_scheduler = LambdaScheduler()

    # degree_normalizer = DegreeNormalizer(stats=datasets[0]._node_degree_stats, device=ConfigRef.config.device)
...
def pretrain_inference(
    ...
    func_ref = FuncRef(
        ...
        load_models=LoadStructuralEncoder(),
        load_optimizers=default_load_optimizers,
        load_scheduler=default_load_scheduler,
    )
    # initialize the ConfigRef which we can call from anywhere
